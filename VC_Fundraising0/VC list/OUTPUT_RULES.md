...
|------|-----|------------|---------|---------|
| /grants | ❌ | DB → filter → renderer | priority_score | 10 |
| /cohorts | ❌ | DB → filter → renderer | priority_score | 10 |
| /funds | ❌ | DB → filter → renderer | priority_score | 10 |
| /all | ❌ | grants 5 + cohorts 5 + funds 5 | priority_score | 15 |
| /ranking | ❌ | DB → batch_rank() | priority_score | 10 |
| /changes | ❌ | change_events DB | detected_at DESC | 10 |
| /org [name] | ✅ | organization_dossiers | - | 1 |
| /fit [name] | ✅ | fit_recommendations + dossier | - | 1 |
| /research [name] | ✅ | research_agent → dossier | - | 1 |
| /brief | ❌ | card_renderer.render_daily_brief() | priority_score | 15 |
| /verify [url] | ✅ | verification_agent | - | 1 |
...
## card_renderer.py 함수 목록

| 함수 | 입력 | 출력 |
|------|------|------|
| render_opportunity_card(opp) | OpportunityCard | str (Telegram 마크다운) |
| render_ranked_list(opps) | list[OpportunityCard] | str |
| render_dossier_card(dossier) | dict | str |
| render_daily_brief(data) | dict | str |

**주의:** 이 함수들의 시그니처 변경 금지 (telegram_bot.py 직접 연결)
...
📁 accelerator_cohort | ✅ verified
⏰ 마감: 2026-04-15 (D-38)
💰 $250K equity-free
🔗 apply: https://alliance.xyz/apply
🎯 fit: 0.82 | priority: 0.79
💡 "분산 AI 인프라 focus, HOOT 섹터 match"
```
